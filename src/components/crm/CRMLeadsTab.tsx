...
export default function CRMLeadsTab() {
  ...
  const visibleLeads = useMemo(() => {
    if (user?.role === 'employee') {
      return leads.filter(l => l.managerId === user.id);
    }
    return leads;
  }, [leads, user]);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Принудительная установка менеджера для сотрудника
    const finalManagerId = user?.role === 'employee' ? user.id : formData.managerId;
        ...
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          ...
          <form onSubmit={handleSubmit} className="space-y-5 mt-4">
            ...
            <div className="space-y-2">
                ...
                <select
                  id="managerId"
                  value={user?.role === 'employee' ? user.id : formData.managerId}
                  onChange={(e) => setFormData({ ...formData, managerId: e.target.value })}
                  className="w-full px-3 py-2 border border-gray-300 rounded-md text-base focus:outline-none focus:ring-2 focus:ring-purple-500 disabled:bg-gray-100 disabled:text-gray-500"
                  required
