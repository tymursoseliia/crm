...
export interface Lead {
  ...
  amountRecycling?: number; // Утиль
  amountFee?: number;       // Госпошлина
  amountDeposit?: number;   // Залоговый платеж
  amountOther?: number;     // Прочее
...
export interface Expense {
  ...
  // Для особых случаев (Жигуль, Клыча)
  isForSpecialEmployee: boolean;
...
export interface SystemSettings {
  ...
  // Постоянные расходы (раз в 2 месяца)
  securityCost: number; // 30000 USDT
  rentCost: number; // 15500 USDT
